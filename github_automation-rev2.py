...
Version: 2.1.0 - With .env proxy support
"""
...
class GitHubSecretSync:
    ...
    def __init__(self, github_token: str, org_name: Optional[str] = None, 
        ...
        # Load proxy configuration from .env if not provided
        ...
        # Handle proxy with embedded credentials
        if self.proxy and not self.proxy_user and '@' in self.proxy:
            # Parse proxy URL that might contain credentials
            parsed = urlparse(self.proxy)
            if parsed.username:
                self.proxy_user = parsed.username
                self.proxy_pass = parsed.password
                # Rebuild proxy URL without credentials for display
                self.proxy_clean = f"{parsed.scheme}://{parsed.hostname}:{parsed.port}"
            else:
                self.proxy_clean = self.proxy
        else:
            self.proxy_clean = self.proxy
    ...
    def _configure_proxy(self) -> Dict:
        ...
        if self.proxy:
            ...
            if self.proxy_user and self.proxy_pass:
                ...
                # Display proxy info (hide password)
                print(f"✓ Proxy configured: {self.proxy_clean if hasattr(self, 'proxy_clean') else self.proxy}")
                print(f"  Using authentication for user: {self.proxy_user}")
            else:
                # Use proxy without authentication
                proxies['http'] = self.proxy
                proxies['https'] = self.proxy
                print(f"✓ Proxy configured: {self.proxy}")
        
        # Also check for HTTPS proxy separately
        https_proxy = os.getenv('HTTPS_PROXY') or os.getenv('https_proxy')
        if https_proxy and https_proxy != self.proxy:
            if self.proxy_user and self.proxy_pass:
            ...
            print(f"✓ HTTPS proxy configured separately")
        
        # Configure no-proxy if specified
        no_proxy = os.getenv('NO_PROXY') or os.getenv('no_proxy')
        if no_proxy:
            os.environ['NO_PROXY'] = no_proxy
            print(f"✓ No-proxy settings: {no_proxy}")
    ...
    def _init_github_client(self):
        ...
        try:
            if self.proxies:
                ...
                # For PyGithub, use custom requester with proxy
    ...
    def secret_exists(self, repo_full_name: str, secret_name: str) -> Tuple[bool, Optional[Dict]]:
        ...
        try:
            ...
            if "407" in str(e):
                print("    💡 Proxy authentication failed. Check your proxy credentials in .env file")
            return False, None
    ...
    def get_repositories(self, repo_filter: Optional[str] = None, language: Optional[str] = None) -> List:
        ...
        try:
            ...
            if "407" in str(e):
                print("💡 Proxy authentication error. Check your .env proxy configuration")
            return []
    ...
    def sync_secrets(self, secrets: Dict[str, str], force_secrets: Optional[List[str]] = None,
                    ...
                    dry_run: bool = False, specific_repo: Optional[str] = None) -> Dict[str, int]:
        ...
        if specific_repo:
            repo = self.get_single_repository(specific_repo)
        ...
        if specific_repo:
            print(f"   Specific repo: {specific_repo}")
        if dry_run:
    ...
    def validate_token(self) -> bool:
        ...
        try:
            ...
            if "407" in str(e):
                print("💡 Proxy authentication failed. Check your .env proxy configuration")
            return False
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Basic usage (proxy from .env file)
  python github_secret_sync.py --secrets-file secrets.txt --dry-run
  
  # Override proxy from command line
  ...
  # List repositories through proxy
  python github_secret_sync.py --list-repos
  
  # Update single repository
  python github_secret_sync.py --repo my-repo --secrets-file secrets.txt
  
  # Language filter with proxy from .env
  python github_secret_sync.py --secrets-file secrets.txt --language python --repo-filter api
        """
    )
    ...
    # Proxy configuration (optional - can override .env)
    parser.add_argument("--proxy", help="Proxy server URL (overrides .env if provided)")
    parser.add_argument("--proxy-user", help="Proxy username (overrides .env if provided)")
    parser.add_argument("--proxy-pass", help="Proxy password (overrides .env if provided)")
    ...
    # Initialize the sync tool with proxy settings (from .env or command line)
